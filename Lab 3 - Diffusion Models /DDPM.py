...
#         T = self.dmconfig.T
#         B = conditions.size(0)
#         h, w = self.dmconfig.input_dim[0], self.dmconfig.input_dim[1]
...
#         conditions = F.one_hot(conditions, num_classes=10).float().to(self.device)

#         with torch.no_grad():  # Ensure no gradients are computed in the sampling process
#             for t in reversed(range(1, T + 1)):
#                 t_tensor = torch.full((B, 1, 1, 1), t, dtype=torch.float, device=self.device)
#                 schedule = self.scheduler(t_tensor)
...
#                 t_normalized = t_tensor / T
#                 cond_predict = self.network(X_t, t_normalized, conditions)
#                 uncond_predict = self.network(X_t, t_normalized, conditions * condition_mask_value)
#                 epsilon_t = (1 + omega) * cond_predict - omega * uncond_predict
                
#                 X_t = oneover_sqrt_alpha * (X_t - (1 - beta_t) / sqrt_oneminus_alpha_bar * epsilon_t) + sigma_t * Z
#             # ==================================================== #
...
class ConditionalDDPM(nn.Module):
    ...
    def scheduler(self, t_s):
        ...
        # get prev alphas
        #maybe using torch.nparange did work but you needed to do torch.tensor(time_steps)
        alpha = 1 - torch.linspace(beta_1, beta_T, steps=T)
        alpha_t_bar_list = torch.cumprod(alpha, dim=0)
    ...
    def forward(self, images, conditions):
        ...
        time_steps = torch.randint(1, T+1, (B,1,1,1))

        schedule = self.scheduler(time_steps)
        sqrt_alpha_bar = schedule['sqrt_alpha_bar'].to('cuda')
        sqrt_oneminus_alpha_bar = schedule['sqrt_oneminus_alpha_bar'].to('cuda')
        #print("sqrt_alpha_bar: ", sqrt_alpha_bar.shape)
        #print("sqrt_oneminus_alpha_bar: ", sqrt_oneminus_alpha_bar.shape)
        ...
        #print("conditions: ", one_hot_cond.shape)
